...
#define Ebwt_INITS					\
	...
	ftabLoHi(
		...
		TIndexOffU& bot)
		{
			top = ftabHi(ftab,eftab,len,ftabLen,eftabLen,i);
			bot = ftabLo(ftab,eftab,len,ftabLen,eftabLen,i+1);
			assert_geq(bot, top);
		}
...
#define WITHIN_FCHR_DOLLARA(x)			\
	...
	constexpr inline TIndexOffU countUpTo(const SideLocus& l, int c) const { // @double-check
		...
		if(l._bp > 0) {
			cCnt += _cCntLUT_4[(int)l._bp][c][side[i]];
		}
		return cCnt;
	}
	...
	constexpr inline void countUpToEx(const SideLocus& l, TIndexOffU* arrs) const {
		...
		for(; i < l._by; i++) {
			arrs[0] += _cCntLUT_4[0][0][side[i]];
			arrs[1] += _cCntLUT_4[0][1][side[i]];
			arrs[2] += _cCntLUT_4[0][2][side[i]];
			arrs[3] += _cCntLUT_4[0][3][side[i]];
		}
		// Count occurences of c in the rest of the byte
		if(l._bp > 0) {
			arrs[0] += _cCntLUT_4[(int)l._bp][0][side[i]];
			arrs[1] += _cCntLUT_4[(int)l._bp][1][side[i]];
			arrs[2] += _cCntLUT_4[(int)l._bp][2][side[i]];
			arrs[3] += _cCntLUT_4[(int)l._bp][3][side[i]];
		}
	}
