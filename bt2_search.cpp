...
static uint16_t nSeedRounds;    // # seed rounds
			...
			for (uint32_t nb=0; nb<batch_parallel_tasks; nb++) {
			  for (uint16_t ib=0; ib<reads_per_batch; ib++) {
				...
				if (irounds[mate]!=nSeedRounds) {
					...
					// no need to load a new one, keep going
					found_unread = true;
					break;
				}
			   ...
			   if (mate_idx[mate]!=MATE_DONE_READING) {
					...
					const uint32_t nrounds = std::min<uint16_t>(nSeedRounds, interval);
...
#endif
			  // Calculate # seed rounds for each mate
			  const uint16_t nrounds = std::min<uint16_t>(nSeedRounds, interval);
